angular.module('myApp.hero-card', ['ngRoute'])

  .config(['$routeProvider', function ($routeProvider) {
    $routeProvider.when('/', {
      templateUrl: 'components/hero-card/hero-card.html',
      controller: 'HeroCtrl'
    });
  }])

  .controller('HeroCtrl', ['$scope', '$http', function ($scope, $http, request) {
    $scope.searchHero = function () {
      ...
      for (i = 0; i < heroName.length; i++) {
        if (!heroName[i].innerHTML.toLowerCase().includes(input)) {
    ...
    // Usei esta função para fazer o hash pedido paraa consumir a api pois nã0o consegui utilizar uma lib para ficar mais simples, isto fica como um ponto para ser melhorado.
